...
def load_data(path, split=0.1):
    ...
    images = sorted(glob(os.path.join(path, "Hearth", "*.png")))
    hearthMask = sorted(glob(os.path.join(path, "ManualMask", "hearthMask", "*.png")))

    split_size = int(len(images) * split)

    train_x, valid_x = train_test_split(images, test_size=split_size, random_state=42)
    train_y1, valid_y1 = train_test_split(hearthMask, test_size=split_size, random_state=42)
    # train_y2, valid_y2 = train_test_split(masks2, test_size=split_size, random_state=42)

    #
    # return (train_x, train_y1), (valid_x, valid_y1)
